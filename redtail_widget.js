...
.rt-tab{padding:8px 24px;border-radius:999px;font-size:13px;cursor:pointer;border:1.5px solid ${CLR.border};background:${CLR.bgCard};color:${CLR.textSub};transition:all .15s;font-family:inherit;font-weight:500;}
...
.rt-selected-summary{background:${CLR.redLight};border:1.5px solid #f5c6c2;border-radius:10px;padding:14px 18px;margin-bottom:20px;font-size:13px;color:${CLR.text};line-height:1.7;}
.rt-selected-summary strong{font-weight:700;}
@media(max-width:680px){
  .rt-w{padding:24px 18px;}
  .rt-title{font-size:20px;}
  .rt-g2,.rt-g3{grid-template-columns:1fr 1fr;}
...
@media(max-width:400px){
  .rt-g2,.rt-g3{grid-template-columns:1fr;}
}
`;
...
window.rtSearch=function(){const oInp=document.getElementById('rt-orig').value.trim(),dInp=document.getElementById('rt-dest').value.trim();const pax=parseInt(document.getElementById('rt-pax').value)||1;const date=document.getElementById('rt-date').value;const err=document.getElementById('rt-serr');const oc=(origSelected||oInp).toUpperCase(),dc=(destSelected||dInp).toUpperCase();const orig=lu(oc),dest=lu(dc);if(!orig){err.textContent=`"${oInp}" not found. Try a city name like "Moab" or code like "SLC".`;return;}if(!dest){err.textContent=`"${dInp}" not found.`;return;}if(oc===dc){err.textContent='Origin and destination must be different.';return;}err.textContent='';tripData={oc,dc,orig,dest,pax,date,isRT:mode==='roundtrip',origName:orig.n,destName:dest.n};selectedAC=null;render('results');};
window.rtBook=function(){if(!selectedAC){document.getElementById('rt-berr').textContent='Please select an aircraft before continuing.';document.getElementById('rt-berr').scrollIntoView({behavior:'smooth',block:'nearest'});return;}
// If we already have lead data, submit and go to thank you
if(leadData){submitHS();render('thankyou');}else{render('lead');}};
window.rtLead=function(){const first=document.getElementById('rt-first').value.trim(),last=document.getElementById('rt-last').value.trim(),email=document.getElementById('rt-email').value.trim(),phone=document.getElementById('rt-phone').value.trim();const err=document.getElementById('rt-lerr');if(!first||!last){err.textContent='Please enter your full name.';return;}if(!email||!/\S+@\S+\.\S+/.test(email)){err.textContent='Please enter a valid email.';return;}if(!phone){err.textContent='Please enter a phone number.';return;}err.textContent='';leadData={first,last,email,phone};submitHS();render('thankyou');};
window.rtRender=function(s){render(s);};

function submitHS(){
  const ac=FLEET.find(a=>a.id===selectedAC);
  const q=ac?calcTotal(ac,tripData.orig,tripData.dest,tripData.pax,tripData.oc,tripData.isRT):null;
  const msg=ac&&q
    ?`Aircraft: ${ac.name} | Route: ${tripData.oc} to ${tripData.dc} | ${tripData.pax} passenger${tripData.pax>1?'s':''} | Date: ${fmtDate(tripData.date)}${tripData.isRT?' (Round trip)':' (One way)'} | Est. total: $${q.total.toLocaleString()} | Flight time: ${fmtT(q.flightTime)}`
    :`Route: ${tripData.oc} to ${tripData.dc} | ${tripData.pax} pax | ${fmtDate(tripData.date)}`;
  fetch(`https://api.hsforms.com/submissions/v3/integration/submit/${HS_PORTAL}/${HS_FORM}`,{
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({fields:[
      {name:'firstname',value:leadData.first},{name:'lastname',value:leadData.last},
      {name:'email',value:leadData.email},{name:'phone',value:leadData.phone},
...
function rSearch(app){const isRT=mode==='roundtrip';app.innerHTML=`
...
<div class="${isRT?'rt-g3':'rt-g2'}">
  ...
  <div class="rt-f"><label>Depart date</label><input type="date" id="rt-date" style="color-scheme:light;"/></div>
  ${isRT?'<div class="rt-f"><label>Return date</label><input type="date" id="rt-retdate" style="color-scheme:light;"/></div>':''}
</div>
<div id="rt-serr" class="rt-err"></div>
<button class="rt-btn" onclick="rtSearch()">See available aircraft &rarr;</button>
<p class="rt-note">Pricing based on Redtail's fleet departing from Canyonlands (KCNY), Moab UT</p>`;
document.getElementById('rt-date').valueAsDate=new Date();
origSelected=null;destSelected=null;dO=-1;dD=-1;
setupSearch('rt-orig','rt-drop-o');setupSearch('rt-dest','rt-drop-d');}

function rResults(app){const{oc,dc,orig,dest,pax,date,isRT,origName,destName}=tripData;const legNm=Math.round(hav(orig.la,orig.lo,dest.la,dest.lo));const avail=FLEET.filter(ac=>calcTotal(ac,orig,dest,pax,oc,isRT)!==null);const unavail=FLEET.filter(ac=>calcTotal(ac,orig,dest,pax,oc,isRT)===null);
const aC=avail.map(ac=>{const q=calcTotal(ac,orig,dest,pax,oc,isRT);return`<div class="rt-card" data-id="${ac.id}" onclick="rtToggle('${ac.id}')"><div class="rt-photos"><img class="rt-photo" src="${IMGS[ac.ext]}" alt="${ac.name} exterior" loading="lazy"/><img class="rt-photo" src="${IMGS[ac.int_]}" alt="${ac.name} interior" loading="lazy"/></div><div class="rt-info"><div class="rt-cell"><span class="rt-cl">Aircraft</span><span class="rt-cv">${ac.name}</span></div><div class="rt-cell"><span class="rt-cl">Type</span><span class="rt-cv">${ac.type}</span></div><div class="rt-cell"><span class="rt-cl">Seats</span><span class="rt-cv">${ac.seats} passengers</span></div><div class="rt-cell g"><span class="rt-cl">Flight time</span><span class="rt-cv t">${fmtT(q.flightTime)}</span></div><div class="rt-cell"><span class="rt-cl">Est. total</span><span class="rt-cv p">$${q.total.toLocaleString()}</span></div></div></div>`;}).join('');
const uC=unavail.length?`<div class="rt-div"><span>Not available for this route</span></div>${unavail.map(ac=>`<div class="rt-card un"><div class="rt-photos"><img class="rt-photo" src="${IMGS[ac.ext]}" loading="lazy"/><img class="rt-photo" src="${IMGS[ac.int_]}" loading="lazy"/></div><div class="rt-info"><div class="rt-cell"><span class="rt-cl">Aircraft</span><span class="rt-cv">${ac.name}</span></div><div class="rt-cell"><span class="rt-cl">Type</span><span class="rt-cv">${ac.type}</span></div><div class="rt-cell"><span class="rt-cl">Seats</span><span class="rt-cv">${ac.seats} passengers</span></div><div class="rt-cell g"></div><div class="rt-cell"><span class="rt-cl">&nbsp;</span><span class="rt-cv na">Not available</span></div></div></div>`).join('')}`:'';
app.innerHTML=`
<div class="rt-rs"><strong>${origName} &rarr; ${destName}</strong>${isRT?' &middot; Round trip':' &middot; One way'}<br>${pax} passenger${pax>1?'s':''} &middot; ${fmtDate(date)} &middot; ${legNm} nm</div>
...
<div class="rt-fn">Prices are estimates. Final quotes confirmed by our team. Rates include aircraft, crew, and fuel.</div>`;}

function rLead(app){const ac=FLEET.find(a=>a.id===selectedAC);const q=ac?calcTotal(ac,tripData.orig,tripData.dest,tripData.pax,tripData.oc,tripData.isRT):null;app.innerHTML=`
<div class="rt-lh"><h2>Almost there</h2><p>Enter your contact details to confirm your booking request. A Redtail team member will follow up personally.</p></div>
${ac&&q?`<div class="rt-selected-summary">
  <strong>${ac.name}</strong> &middot; ${tripData.origName} &rarr; ${tripData.destName}<br>
  ${tripData.pax} passenger${tripData.pax>1?'s':''} &middot; ${fmtDate(tripData.date)}${tripData.isRT?' &middot; Round trip':''} &middot; <strong>Est. $${q.total.toLocaleString()}</strong>
</div>`:''}
<div class="rt-g2" style="margin-bottom:16px">
  <div class="rt-f"><label>First name</label><input type="text" id="rt-first" placeholder="John"/></div>
  <div class="rt-f"><label>Last name</label><input type="text" id="rt-last" placeholder="Smith"/></div>
</div>
<div class="rt-f" style="margin-bottom:16px"><label>Email address</label><input type="email" id="rt-email" placeholder="john@example.com"/></div>
<div class="rt-f"><label>Phone number</label><input type="tel" id="rt-phone" placeholder="(555) 000-0000"/></div>
...
function rThankyou(app){const ac=FLEET.find(a=>a.id===selectedAC);const q=ac?calcTotal(ac,tripData.orig,tripData.dest,tripData.pax,tripData.oc,tripData.isRT):null;app.innerHTML=`
<div class="rt-ty">
  <div class="ic">&#9992;&#65039;</div>
  <h3>We'll be in touch soon</h3>
  <p>Thanks${leadData?.first?', '+leadData.first:''}. ${ac&&q?`Your booking request for the <strong>${ac.name}</strong> from ${tripData.origName} to ${tripData.destName} on ${fmtDate(tripData.date)} has been received. Estimated total: <strong>$${q.total.toLocaleString()}</strong>.`:''}</p>
  <p style="margin-top:16px">Want to talk now?<br><a href="tel:4352597421">(435) 259-7421</a></p>
